...
def calculate(request_handler, response_handler):
    if request_handler.check_parameter_status():
        calculator = calc.Calculator()
        calculator.calculate(request_handler, response_handler)

def create_param_frame(container, request_handler):
    ...
    apply_date_button = ttk.Button(frame, text="Use This Date",
        ...
        request_handler.set_date(cal.get_date())
        ])
    ...
    latitude_entry.bind('<FocusOut>', (lambda event, lat=latitude: request_handler.set_latitude(lat.get())))

    latitude_combobox = ttk.Combobox(frame, values=['N', 'S'], width=1, state='readonly')
    latitude_combobox.set(request_handler.get_latitude_dir())
    latitude_combobox.grid(ipadx=3, row=4, column=3, sticky='w')
    latitude_combobox.bind('<<ComboboxSelected>>', (lambda event, cb=latitude_combobox: request_handler.set_latitude_dir(cb.get())))
    ...
    longitude_entry.bind('<FocusOut>', (lambda event, lng=longitude: request_handler.set_longitude(lng.get())))

    longitude_combobox = ttk.Combobox(frame, values=['W', 'E'], width=1, state='readonly')
    longitude_combobox.set(request_handler.get_longitude_dir())
    longitude_combobox.grid(ipadx=3, row=5, column=3, sticky='w')
    longitude_combobox.bind('<<ComboboxSelected>>', (lambda event, cb=longitude_combobox: request_handler.set_longitude_dir(cb.get())))
...
def create_main_window():
    ...
    request_handler = req.RequestHandler()
